...
require_once PROJECT_ROOT_PATH . DIRECTORY_SEPARATOR . 'api' . DIRECTORY_SEPARATOR . 'model' . DIRECTORY_SEPARATOR . 'TeamModel.php';
...
class TeamController extends BaseController
{
    ...
    public function oneAction($id = [''])
    {
        ...
        if (strtoupper($requestMethod) == 'GET') {
            try {
                ...
                $teamModel = new TeamModel();
                $arrTeams = $teamModel->getOneTeam($id);
                $responseData = json_encode($arrTeams);
            } catch (\Error $e) {
                $strErrorDesc = $e->getMessage() . 'Something went wrong! Please contact support.';
                $strErrorHeader = 'HTTP/1.1 500 Internal Server Error';
    ...
    /** 
     * "/team/bysplit" Endpoint - un jugador
     */
    public function bysplitAction($id = [''])
    {
        $strErrorDesc = '';
        $requestMethod = $_SERVER["REQUEST_METHOD"];
        ...
        if (strtoupper($requestMethod) == 'GET') {
            try {
                $id = str_replace('id=', '', $id[0]);
                $teamModel = new TeamModel();
                $arrTeams = $teamModel->getTeamsBySplitId($id);
                $responseData = json_encode($arrTeams);
            } catch (\Error $e) {
                $strErrorDesc = $e->getMessage() . 'Something went wrong! Please contact support.';
                $strErrorHeader = 'HTTP/1.1 500 Internal Server Error';
